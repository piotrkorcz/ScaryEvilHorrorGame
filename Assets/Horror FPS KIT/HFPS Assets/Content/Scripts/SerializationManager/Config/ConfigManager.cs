...
namespace ThunderWire.Configuration {
	public class ConfigManager {
		...
		public List<string> ConfigKeysCache = new List<string>();
					...
					while (reader.Peek() != -1) {
                        ...
                        if (!string.IsNullOrEmpty(line))
                        {
                            ...
                            if (m_Section == "" || m_Key == "" || m_Value == "")
                                continue;
                            //Debug.Log("Section: " + m_Section + "; Key: " + m_Key + "; Value: " + m_Value);
		...
		private static void WriteDictionary() {
            ...
            using (StreamWriter sw = new StreamWriter (filename)) {
				foreach (KeyValuePair<string, Dictionary<string, string>> Section in ConfigDictionary) {
                    ...
                    if (divideSections) { sw.WriteLine(""); } else { divideSections = true; }
